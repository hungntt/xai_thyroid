...
class KDE(object):
    def __init__(self, sess, image, j, y_p_num_detections, y_p_boxes):
        self.box, self.box_predicted = [], []
        self.image = image
        self.h_img, self.w_img = image.shape[1:3]
        self.sess = sess
        self.y_p_num_detections = y_p_num_detections
        self.y_p_boxes = y_p_boxes
        self.image_name = os.path.basename(j)

    def get_kde_map(self):
        """
        ...
        """
        x_box, y_box = get_center(self.box)
    ...
    def get_kde_score(self, kde_kernel):
        """
        ...
        """
        x_predict, y_predict = get_center(self.box_predicted)
    ...
    def show_kde_map(self):
        ...
        ax.set_xlabel('Y1')
        ax.set_ylabel('Y0')
        plt.savefig('kde_' + self.image_name + '_kde.jpg', dpi=600)
        plt.show()
        ...
        ax.set_xlabel('Y1')
        ax.set_ylabel('Y0')
        plt.savefig('kde_' + self.image_name + '_kde_blue.jpg', dpi=600)
        plt.show()
    ...
    def get_box_predicted(self, img_input):
        ...
        for i in range(num_box):
            ...
            self.box_predicted.append([x1, y1, x2, y2])

        iou = []
        for i in range(num_box):
            for j in range(300):
                iou.append(bbox_iou(self.box[j], self.box_predicted[i]))

        return self.box, self.box_predicted
